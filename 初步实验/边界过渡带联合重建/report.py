"""依据保存的完整对照结果生成数学依据、消融、动态验收与硬件边界报告。"""
...
def main():
    ...
    audit = json.loads((OUT/'independent_audit.json').read_text(encoding='utf-8'))
    hardware = json.loads((OUT/'hardware.json').read_text(encoding='utf-8'))
    labels = dict(fixed='固定边界基线', refine_only='只细化内部', expand_only='只扩展维护域', joint='联合方法')
    table = []
    for method in data['methods']:
        last = method['history'][-1]
        table.append(f"|{labels[method['name']]}|{last['removed_faces']}|{last['min_angle_deg']:.3f}|"
                     f"{last['error_max_mm']:.5f}|{last['bad_faces']}|{len(method['history'])}|"
                     f"{method['elapsed_ms']:.0f}|{'通过' if last['accepted'] else '拒绝'}|")
    sequence_rows = []
    for sequence in data['sequences']:
        passed = [r for r in sequence['records'] if r['accepted']]
        angle = f"{min(r['min_angle_deg'] for r in passed):.3f}" if passed else '—'
        bound = f"{max(r['error_bound_mm'] for r in passed):.5f}" if passed else '—'
        times = [r['total_ms'] for r in sequence['records']]
        sequence_rows.append(f"|{sequence['z']}|{sequence['accepted_steps']}/16|{sequence['changed_steps']}|"
            f"{sequence['max_removal_mm']:.5f}|{angle}|{bound}|{np.mean(times):.1f}|{np.percentile(times, 95):.1f}|"
            f"{np.percentile(times, 99):.1f}|{max(times):.1f}|")
    medium = next(s for s in data['sequences'] if s['z'] == 2.5)
    means = {k: np.mean([r[k] for r in medium['records']])
             for k in ('total_ms', 'pipeline_ms', 'certificate_ms', 'height_ms')}
    now = datetime.now(timezone(timedelta(hours=8))).strftime('%Y-%m-%d %H:%M:%S')
    report = f'''# 19-边界过渡带联合重建与整骨逐步验收报告

> **生成时间**：{now}（北京时间）
> **修改时间及修改内容**：{now}，首次生成；记录联合重建、已有方法基线与消融、两条整骨磨削序列、深切拒绝及核显核验。
...
|---|---|---|---|---|
{chr(10).join(f"|{r['z']}|{r['sample_mean_mm']:.6f}|{r['sample_p95_mm']:.6f}|{r['sample_p99_mm']:.6f}|{r['sample_max_mm']:.6f}|" for r in audit)}

全部采样误差位于对应末态证书内。该交叉检查独立于图域定位实现，但仍共享水平扫掠模型，不是独立精确三维布尔参照；正式比较仍需补充。三角质量门槛仅适用于重建及过渡区域，未修改全骨的既有差面不因此消失。

## 五、性能与核显核验

CPU为Intel Core Ultra 5 225H，Python {data['python']}，系统{data['platform']}。正式矩阵是一次运行，不提供重复计时置信区间；上表是序列内逐步统计，包含拒绝请求，不能解释为多次独立实验的分布。

中等深度显式计时均值：单步含整骨检查{means['total_ms']:.1f} ms、局部更新管线{means['pipeline_ms']:.1f} ms、证书计算{means['certificate_ms']:.1f} ms、高度查询{means['height_ms']:.1f} ms。高度查询多数发生在证书计算内，属于嵌套时间，禁止将四项相加。以上不含初始化、文件保存、状态风险字段与渲染，**未达100 ms目标，也不是完整系统端到端延迟**。

探索性cProfile记录中出现累计时间小于自身时间等不一致，未据此形成性能结论；保留cpu_profile文件供追查，正式结论使用显式perf_counter计时。profile_run.py可对已保存合格初始网格单独复测。

本机只读核验：GPU设备名 `{hardware['devices'][0]['name']}`，驱动 `{hardware['devices'][0]['driver']}`，OpenCL版本 `{hardware['devices'][0]['version'].strip()}`，驱动声明cl_khr_fp64支持。设备名中的16GB不能解释为独立显存容量；WMI AdapterRAM也不用于推断可用显存。本次未安装GPU驱动、CUDA、PyOpenCL或新Conda/Docker环境，未编译或运行GPU算子。
...
'''
    target = ROOT/'课题规划与专题调研/19-边界过渡带联合重建与整骨逐步验收报告.md'
