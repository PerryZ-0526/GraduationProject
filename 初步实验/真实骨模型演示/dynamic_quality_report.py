"""从逐步实验原始记录生成中文报告及可复现曲线，不填充未完成步骤。"""
import json
...
def main():
    ...
    rows = data['rows']
    passed = [x for x in data['attempts'] if x['accepted']]
    failed = [x for x in data['attempts'] if not x['accepted']]
    latency = np.array([r['pipeline_ms'] for r in rows])
    drift = np.array([r['reference_max_mm'] for r in rows])
    bad = np.array([r['quality_bad_pct'] for r in rows])
    plt.rcParams['font.sans-serif'] = ['Microsoft YaHei']
    fig, axes = plt.subplots(3, 1, figsize=(10, 8), sharex=True)
    steps = [r['step'] for r in rows]
    for ax, values, label, limit in zip(axes, [bad, drift, latency],
                                       ['全骨差面占比（%）', '双向抽样距离最大值（mm）', '计算管线耗时（ms）'],
                                       [1, .1, 100]):
        ax.plot(steps, values, marker='.', label='已发布步骤')
        ax.axhline(limit, color='red', linestyle='--', label='当前实验门槛/目标')
        ax.set_ylabel(label)
        ax.grid(alpha=.25)
        ax.legend(loc='upper left')
    axes[-1].set_xlabel('磨削步骤（被拒绝及未执行步骤不补点）')
    fig.suptitle(f"逐步维护实测：发布{len(rows)}/138步；非实时合格证明")
    fig.tight_layout()
    fig.savefig(out/'逐步质量与延迟.png', dpi=150)
    plt.close(fig)
    timestamp = datetime.now(ZoneInfo('Asia/Shanghai')).strftime('%Y-%m-%d %H:%M:%S')
    rejection = failed[-1]['error'] if failed else '本次未拒绝'
    timing = '\n'.join(f"| {key} | {np.mean([r[key] for r in rows]):.2f} |" for key in
                       ['tool_ms', 'update_ms', 'export_ms', 'metrics_ms', 'maintain_ms', 'check_ms', 'pipeline_ms'])
    text = f'''# 11-磨削过程逐步网格质量维护实验与未达标问题报告

> **生成时间**：{timestamp}（北京时间）
> **修改时间及修改内容**：{timestamp}，根据逐步维护、回灌、拒绝回滚和窗口实测首次生成。
...
原始记录：`初步实验/真实骨模型演示/逐步质量维护实验/results.json`；每轮时间戳run文件用于保留结果。记录包含北京时间、参数、代码摘要、逐步质量、双向距离的均值/P95/P99/最大值及超限样本数、耗时与拒绝原因。当前代码SHA256：`{data.get('code_sha256', '旧记录未存储')}`。

## 四、实测结果与失败证据

本轮记录时间：{data['time_beijing']}。

| 指标 | 实测 |
| --- | --- |
| 发布步数/计划步数 | {len(rows)}/138 |
| 拒绝事件数 | {len(failed)}（首次失败即停，不是其余步骤均通过） |
| 全骨差面占比范围 | {bad.min():.6f}%–{bad.max():.6f}% |
| 已发布邻域差面占比最大值 | {max(a['local_bad_pct'] for a in passed):.6f}% |
| 已发布步骤中最小q | {min(a['quality']['q_min'] for a in passed):.9g} |
| 已发布步骤中最小角 | {min(a['quality']['angle_min_deg'] for a in passed):.9g}° |
| 累计双向抽样距离最大值 | {drift.max():.9f} mm |
| 计算耗时均值/P95/P99/最大值 | {latency.mean():.2f}/{np.percentile(latency,95):.2f}/{np.percentile(latency,99):.2f}/{latency.max():.2f} ms |
| 超过100 ms目标 | {int(np.sum(latency>100))}/{len(rows)}步 |
...
'''
    report = base.parents[1]/'课题规划与专题调研'/'11-磨削过程逐步网格质量维护实验与未达标问题报告.md'
